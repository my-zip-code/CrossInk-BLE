...
constexpr unsigned long FREE2_STALE_RELEASE_DEFAULT_MS = 120;
constexpr unsigned long FREE2_STALE_RELEASE_READER_MS = 120;
constexpr unsigned long FREE2_REPEAT_SUPPRESS_MS = 650;
constexpr const char* BLE_BONDED_DEVICE_FILE = "/.crosspoint/ble_bonded_device.txt";
}
...
void BluetoothHIDManager::onHIDNotify(NimBLERemoteCharacteristic* pChar, uint8_t* pData, size_t length, bool isNotify) {
  ...
  if (free2Profile && isNewPressEvent && device->lastInjectionTime > 0 &&
      (nowMs - device->lastInjectionTime) < FREE2_REPEAT_SUPPRESS_MS) {
    isNewPressEvent = false;
    if (g_instance->_debugCaptureEnabled) {
      LOG_INF("BTDBG", "Suppressed Free2 repeat key=0x%02X dt=%lu", keycode,
              nowMs - device->lastInjectionTime);
    }
  }
...
bool BluetoothHIDManager::hasBondedDevice() const {
  return !_bondedDeviceAddress.empty();
}

std::string BluetoothHIDManager::getBondedDeviceName() const {
  return _bondedDeviceName;
}

bool BluetoothHIDManager::connectToBondedDevice() {
  if (!_enabled) {
    if (!enable()) {
      return false;
    }
  ...
  if (_bondedDeviceAddress.empty()) {
    lastError = "No saved BLE device";
    return false;
  }

  LOG_INF("BT", "Connecting to saved BLE device: %s (%s)",
          _bondedDeviceAddress.c_str(), _bondedDeviceName.c_str());

  return connectToDevice(_bondedDeviceAddress);
}
...
void BluetoothHIDManager::saveState() {
  ...
  std::string data;
  data += _bondedDeviceAddress;
  data += "\n";
  data += _bondedDeviceName;
  data += "\n";

  Storage.writeFile(BLE_BONDED_DEVICE_FILE, data.c_str());
  LOG_INF("BT", "Saved bonded BLE device: %s (%s)",
          _bondedDeviceAddress.c_str(), _bondedDeviceName.c_str());
}

void BluetoothHIDManager::loadState() {
  if (!Storage.exists(BLE_BONDED_DEVICE_FILE)) {
    LOG_DBG("BT", "No bonded BLE device file found");
    return;
  }

  String content = Storage.readFile(BLE_BONDED_DEVICE_FILE);
  content.trim();

  if (content.isEmpty()) {
    _bondedDeviceAddress.clear();
    _bondedDeviceName.clear();
    LOG_DBG("BT", "Bonded BLE device file is empty");
    return;
  }

  int newline = content.indexOf('\n');
  if (newline < 0) {
    _bondedDeviceAddress = content.c_str();
    _bondedDeviceName.clear();
  } else {
    String address = content.substring(0, newline);
    String name = content.substring(newline + 1);
    address.trim();
    name.trim();
    _bondedDeviceAddress = address.c_str();
    _bondedDeviceName = name.c_str();
  }

  LOG_INF("BT", "Loaded bonded BLE device: %s (%s)",
          _bondedDeviceAddress.c_str(), _bondedDeviceName.c_str());
}
